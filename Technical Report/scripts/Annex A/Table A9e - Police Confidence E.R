...
# SSCQ by gen health
sscq_estimates <- svymean(~PolConE, survey_ind, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(PolConE = levels(analyse23_hh$PolConE), .before = mean) %>%
...
survey_estimates <- analyse23_hh[c('PolConE', 'survey', 'cluster', 'pooled_crim_wt', 'LA')] 
...
scjs_estimates <- svymean(~PolConE, scjs_ind, na.rm = TRUE) %>%
  as_tibble() %>%
  mutate(PolConE = levels(scjs_estimates$PolConE), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
#shes_estimates <- svymean(~PolConE, shes_ind, na.rm = TRUE) %>%
 # as_tibble() %>%
  #mutate(PolConE = levels(shes_estimates$PolConE), .before = mean) %>%
  #mutate(lower_ci = (mean - 1.96 * SE) *100,
   #      upper_ci = (mean + 1.96 * SE) *100,
    #     perc = mean *100,
     #    margin_of_error = upper_ci - perc,
...
shs_estimates <- svymean(~PolConE, shs_ind, na.rm = TRUE) %>%
  as_tibble() %>%
  mutate(PolConE = levels(shs_estimates$PolConE), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
table_a9e <- bind_rows(sscq_estimates, scjs_estimates, #shes_estimates,
                       shs_estimates)
...
table_a9e$Survey <- Survey

table_a9e <- table_a9e %>% select(Survey, PolConE, perc_with_ci)
table_a9e

# Visualise estimates ------------------------------------------------------------

table_a9e <- table_a9e %>%
  select(Survey, PolConE, perc_with_ci) %>%
  group_by(Survey) %>% 
  pivot_wider(
    names_from = PolConE,
...
kable(table_a9e,
      caption = "Table A.9.E: Confidence in the police to solve crimes by source survey (row % and margin of error)")


saveRDS(table_a9e, 
        "Technical Report/output/table_a9e_estimates.rds")

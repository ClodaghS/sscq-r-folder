...
sscq_23 <- svymean(~IndCare, survey_23, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(IndCare = levels(sscq23$IndCare), .before = mean) %>%
...
sscq_22 <- svymean(~IndCare, survey_22, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(IndCare = levels(sscq22$IndCare), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
sscq_19 <- svymean(~IndCare, survey_19, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(IndCare = levels(sscq19$IndCare), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
sscq_18 <- svymean(~IndCare, survey_18, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(IndCare = levels(sscq18$IndCare), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
IndCare <- bind_rows(sscq_23, sscq_22, sscq_19, sscq_18)
...
saveRDS(IndCare_table, 
        "QA/Outputs/IndCare_table.rds")

# by IndCare
# Yes
IndCare1 <- IndCare %>% filter(IndCare == 'Yes')
(IndCare_1 <- ggplot(IndCare1, aes(x = Year, y = perc)) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "SSCQ Respondent Unpaid Care 
    Provision by Year (Yes)"))

#ggsave(IndCare_1, filename = "QA/Outputs/IndCare1.pdf",
 #      width = 200, height = 120, units = "mm")

# No
IndCare2 <- IndCare %>% filter(IndCare == 'No')
(IndCare_2 <- ggplot(IndCare2, aes(x = Year, y = perc)) +
  geom_boxplot(fill = sg_colour_values["dark-blue"])+
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.5,
                colour = sg_colour_values[4]) +
  labs(
...
#ggsave(IndCare_2, filename = "QA/Outputs/IndCare2.pdf",
 #      width = 200, height = 120, units = "mm")

...
sscq_23 <- svymean(~ILOEmp, survey_23, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(ILOEmp = levels(sscq23$ILOEmp), .before = mean) %>%
...
sscq_22 <- svymean(~ILOEmp, survey_22, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(ILOEmp = levels(sscq22$ILOEmp), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
sscq_19 <- svymean(~ILOEmp, survey_19, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(ILOEmp = levels(sscq19$ILOEmp), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
sscq_18 <- svymean(~ILOEmp, survey_18, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(ILOEmp = levels(sscq18$ILOEmp), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
ILOEmp <- bind_rows(sscq_23, sscq_22, sscq_19, sscq_18)
...
saveRDS(ILOEmp_table, 
        "QA/Outputs/ILOEmp_table.rds")

# by Employment
# In employment
ILOEmp1 <- ILOEmp %>% filter(ILOEmp == 'In employment')
(ILOEmp_1 <- ggplot(ILOEmp1, aes(x = Year, y = perc)) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "SSCQ Respondent Employment 
    by Year (In employment)"))

#ggsave(ILOEmp_1, filename = "QA/Outputs/ILOEmp1.pdf",
 #      width = 200, height = 120, units = "mm")

# ILO unemployed
ILOEmp2 <- ILOEmp %>% filter(ILOEmp == 'ILO unemployed')
(ILOEmp_2 <- ggplot(ILOEmp2, aes(x = Year, y = perc)) +
  geom_boxplot(fill = sg_colour_values["dark-blue"])+
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.5,
                colour = sg_colour_values[4]) +
  labs(
    ...
    y = NULL,
    title = "SSCQ Respondent Employment
    by Year (ILO unemployed)"))

#ggsave(ILOEmp_2, filename = "QA/Outputs/ILOEmp2.pdf",
 #      width = 200, height = 120, units = "mm")

# Inactive
ILOEmp3 <- ILOEmp %>% filter(ILOEmp == 'Inactive')
(ILOEmp_3 <- ggplot(ILOEmp3, aes(x = Year, y = perc)) +
  geom_boxplot(fill = sg_colour_values["dark-blue"])+
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.5,
                colour = sg_colour_values[4]) +
  labs(
...
#ggsave(ILOEmp_3, filename = "QA/Outputs/ILOEmp3.pdf",
 #      width = 200, height = 120, units = "mm")

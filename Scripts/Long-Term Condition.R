...
sscq23 <- sscq23 %>% 
  ...
  mutate(LTCondition = factor(LTCondition, levels = c(-1, 1, 2),
                              labels = c(NA,
                                         "Yes",
                                         "No")))
...
sscq_23 <- svymean(~LTCondition, survey_23, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(LTCondition = levels(sscq23$LTCondition), .before = mean) %>%
...
sscq_22 <- svymean(~LTCondition, survey_22, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(LTCondition = levels(sscq22$LTCondition), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
sscq_19 <- svymean(~LTCondition, survey_19, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(LTCondition = levels(sscq19$LTCondition), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
sscq_18 <- svymean(~LTCondition, survey_18, na.rm = TRUE)%>%
  as_tibble() %>% 
  mutate(LTCondition = levels(sscq18$LTCondition), .before = mean) %>%
  mutate(lower_ci = (mean - 1.96 * SE) *100,
         upper_ci = (mean + 1.96 * SE) *100,
         perc = mean *100,
         margin_of_error = upper_ci - perc,
...
LTCondition <- bind_rows(sscq_23, sscq_22, sscq_19, sscq_18)
...
LTCondition$Year <- Year

LTCondition_table <- LTCondition %>% select(Year, LTCondition, perc_with_ci)
LTCondition_table

# Visualise estimates ------------------------------------------------------------
LTCondition_table <- LTCondition_table %>% 
  group_by(Year) %>% 
  pivot_wider(
    names_from = LTCondition,
    values_from = perc_with_ci)

print(LTCondition_table)

saveRDS(LTCondition_table, 
        "QA/Outputs/LTCondition_table.rds")

# by LTCondition
# Yes
LTCondition1 <- LTCondition %>% filter(LTCondition == 'Yes')
(LTCondition_1 <- ggplot(LTCondition1, aes(x = Year, y = perc)) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "SSCQ Respondent Long-Term 
    Condition by Year (Yes)"))

#ggsave(LTCondition_1, filename = "QA/Outputs/LTCondition1.pdf",
 #      width = 200, height = 120, units = "mm")

# No
LTCondition2 <- LTCondition %>% filter(LTCondition == 'No')
(LTCondition_2 <- ggplot(LTCondition2, aes(x = Year, y = perc)) +
  geom_boxplot(fill = sg_colour_values["dark-blue"])+
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.5,
                colour = sg_colour_values[4]) +
  labs(
    ...
    y = NULL,
    title = "SSCQ Respondent Long-Term 
    Condition by Year (No)"))

#ggsave(LTCondition_2, filename = "QA/Outputs/LTCondition2.pdf",
 #      width = 200, height = 120, units = "mm")
